...
{ 
  ...
  pair<pair<key_T,val_T>,bool> insert(pair<key_T, val_T>); 
  void erase(key_T); 
...
template <typename key_T, typename val_T>
pair<pair<key_T,val_T>,bool> Map<key_T, val_T>::insert(pair<key_T, val_T> p) 
{ 
  // key_T key = p.first;
  // val_T val = p.second;
  //pair<pair<key_T,val_T>&,bool>
  ...
  */
  ...
  { 
    ...
    return make_pair(n->value, true);
    slsize++; 
  }else{
    cout<<"key already there"<<endl;
    return make_pair(current->value, false);
  } 
}; 
